...
#ifdef SCOUNIX
#include <stdio.h>
void ultoa(unsigned long Value,char* String,int)
{
 ...
 switch( pt->fldtype )
  {
   ...
   // No conversion
   case FLDTYPE_STRING:   if ( strlen(pt->type.string) >= maxlen )
                         return false;
 ...
 switch( pt->fldtype )
  {
   ...
   // STRING: move string to caller's buffer and set length
   case FLDTYPE_STRING:  len=strlen(pt->type.string);
                          if ( buflen <
...
//----------------------------------------------------
bool IPCMessage::SendMsg(char* ProcessName,WORD TimeOut,
 ...
 if ( (pt = (FLDPTR *)getfld(fn)) != NULL )
  {
   ...
   if ( pt->fldtype != fldtype ||
        ...
        (fldtype == FLDTYPE_STRING && fldlen != strlen(pt->type.string)) )
     DelFld(fn);
   else
    {
     switch(fldtype)
