// Adds a Program to the Startup Group
...
void main(int argc,char* argv[])
{
 ...
 if ( argc != 3 )
  {
   cout << "Usage is AddStartup ProgramFile ProgramName\n";
   exit(EXIT_FAILURE);
  }
 ...
 strcpy(commands,"[DeleteGroup(Group)]");
 len=strlen(commands);
 strcat(commands,"[DeleteGroup(Testing123)]");
 len=strlen(commands);
 if ( ! DdeClientTransaction((LPBYTE)commands,
               len, HConv, 0L, CF_TEXT, XTYP_EXECUTE, 5000, 0) )
  {
   cout << "DDE TRANSACTION FAILED\n";
   quit(EXIT_FAILURE);
  }
