...
#define ACTROW(n)\
 ...
 ActRows[n-1].IDS=new TEdit(this,IDC_ACT##n##IDS,5);\
 ActRows[n-1].TIME=new TEdit(this,IDC_ACT##n##TIME,7)
#define CHECKSROW(n)\
 ...
 CheckRows[n-1].IDS=new TEdit(this,IDC_CK##n##IDS,5)
 ...
 for (i=0; i<MAXACTIVITY; ++i)
  {
   ...
   ActRows[i].TIME->Transfer(Buf,tdGetData);
   Act[i].Time = _atoi(&Buf[0],2) * 3600 +
                 _atoi(&Buf[2],2) * 60 + _atoi(&Buf[4],2);
   ...
   ActRows[i].IDS->Transfer(Buf,tdGetData);
   if ( Buf[0] )
     memcpy(Act[i].Ids,Buf,sizeof(Act[i].Ids));
  }
 ...
 for (i=0; i<MAXCHECKS; ++i)
  {
   ...
   if ( ! Buf[0] )
    break;
   memcpy(Check[i].CheckDate,Buf,sizeof(Check[i].CheckDate));
   ...
   CheckRows[i].IDS->Transfer(Buf,tdGetData);
   memcpy(Check[i].Ids,Buf,sizeof(Check[i].Ids));
  }
 if ( i )
  Msg.PutFld(FN_BADCHECKS,&Check,sizeof(BADCHECK)*i);
}
...
void ResponseDialog::ProcessMessage(IPCMessage& Msg)
{
 ...
 char Buf[100];
 MOTHERSNAME->Transfer(Buf,tdGetData);
 Msg.PutFld(FN_MOTHERSNAME,Buf);
}
