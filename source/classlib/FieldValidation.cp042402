...
//logg1(__LINE__, "In IsValidLicense()-state string is",License, lState, 0);
    ...
    if( (iReturn == ALPHA_COUNT_ERROR)   ||
     ...
     {
//logg1(__LINE__, "Fail IsValidLicense()-License is",License, lState, iReturn);
        return false;
     }
...
// This function validates licenses acording to values in the
...
static int isValidTableLicense(char* License, long lState, int* piMoreFlag)
{
    ...
    if(!( iStateDataHasBeenReadFromFile))  // file has not been read yet
    {
       ...
       if(iReturn == 1)  // then try for a local copy
       {
...
YT - Yukon Territory
    ...
    MaxC - Maximum characters in License Number
    MinC - Minimum characters in License Number
    MaxA - Maximum Alpha characters in License Number
    MinA - Minimum Alpha characters in License Number
    Lead - Is there a leading Alpha character in License Number
...
#if VERSION_060900
// Moved to CheckAuth.CPP where it belongs
// MAB060999
// This makes sure that the relationship between the Bank Number and the
...
// MAB051200  won't work, has to be at handoff from Kenwood
// if(iLenBankNumber > 9)
//    return false;

 iReturn = GetFld(Fn,szBankNumber,EditLen);
 szBankNumber[iLenBankNumber - 1] = '\0';
 //logg1(__LINE__, "ValidRouteAcct()-BankNum is",szBankNumber,FN_BANKNUMBER,iLenBankNumber);
 ...
 //logg1(__LINE__, "ValidRouteAcct()-BankAccount is",szBankAccount,FN_BANKACCOUNT,iLenBankAccount);
...
//----------------------------------------------------------------------
//static int CheckAuth::isValidTableBankNumber(char* pszBankNumber,char *pszBankAccount)
int CheckAuth::isValidTableBankNumber(char* pszBankNumber,char *pszBankAccount)
{
    ...
    //logg1(__LINE__, "Start Validation","",0,0);
    ...
    if(!( siDataHasBeenReadFromFile))  // file has not been read yet
    {
...

      {
         ...
         //logg1(__LINE__, "File open fail on G Drive","",0,0);
      }
      if(iReturn)
      {
         iReturn = 0;
         ...
         {
            iReturn = FILE_OPEN_ERROR;
            //logg1(__LINE__, "File open fail on D drive","",0,0);
         }
      }
      if(iReturn)
      {
         ...
         {
            iReturn = FILE_OPEN_ERROR;
            //logg1(__LINE__, "File open fail in . directory","",0,0);
         }
      }
      ...
      for(ii = 0; ii < NUM_ROUTES_TO_READ_FROM_FILE; ii++)
      {
         ...
         //logg1(__LINE__, "Buf is",szbuf,0,0);
         ...
         //logg1(__LINE__, "Test dat","",dat[ii].lBankNumber,ii);
    ...
    //logg1(__LINE__, "Validation","Past Read File",lBankNumber,0);
    // find data for Bank Route
    for(iCount = 0; iCount < siRecordCount; iCount++)
    {
       //logg1(__LINE__, "Test in ","",lBankNumber,iCount);
       //logg1(__LINE__, "Test dat","",dat[iCount].lBankNumber,siRecordCount);
