...
//
// MAB100300
// Delay for 2 seconds on sendHoldCheckDecline
//
// MAB061001
// Make Bank Account enabled in voiceAuth
//
// MAB072302
...
// Do by adding 4 unsigned long values to the Class definition
...
// logg(__LINE__, "In chkauth constructor()", "",0,0);
}
...
// MAB061001
#if 0
 ENABLE(MICR,FN_BANKACCOUNT);  // Try 1
#else
...
void TCheckAuthDlgClient::doDecision()
{
 ...
 char BufLast[20];
 char szMsgBuf[128];
 unsigned long  ulFirstApproval;
 unsigned long  ulLastApproval;
#endif
...
#if 0
 ulFirstApproval = AuthMsg->FirstApproval;
 ulLastApproval  = AuthMsg->LastApproval;

// logg(__LINE__, "In chkauthdlg.cpp doAuthorization()", "First approval",(long)AuthMsg->FirstApproval,0);
// logg(__LINE__, "In chkauthdlg.cpp doAuthorization()", "Last Approval",(long)AuthMsg->LastApproval,0);
// logg(__LINE__, "In chkauthdlg.cpp doAuthorization()", "Amt and sum",(long)AuthMsg->AmountApprovals, (int)AuthMsg->NumApprovals);

 if(AuthMsg->FirstApproval == 0)
 {
   strcpy(szMsgBuf, "No Positive File Information Available");
 }
 else
 {
   t=localtime( (time_t*)&ulFirstApproval);
   sprintf(BufFirst,"%02d/%02d/%02d",t->tm_mon+1,t->tm_mday,t->tm_year%100);
//   logg(__LINE__, "In chkauthdlg.cpp doAuthorization() First approval",BufFirst,0,0);
   t=localtime( (time_t*)&ulLastApproval);
   sprintf(BufLast,"%02d/%02d/%02d",t->tm_mon+1,t->tm_mday,t->tm_year%100);
//   logg(__LINE__, "In chkauthdlg.cpp doAuthorization() Last approval",BufLast,0,0);

   sprintf(szMsgBuf,"First Check - %s\n\nLast Check - %s\n\nNumber Approved - %d\n\nAmount Approved - %ld",
                  					   BufFirst,BufLast,AuthMsg->NumApprovals,
                  					   AuthMsg->AmountApprovals);

//    logg(__LINE__, "In chkauthdlg.cpp doAuthorization()",szMsgBuf,(long)AuthMsg->AmountApprovals, (int)AuthMsg->NumApprovals);
  }
  MessageBox(szMsgBuf,"Positive File Results",MB_OK);
  AuthMsg->FirstApproval = 0;
  AuthMsg->LastApproval = 0;
  AuthMsg->NumApprovals = 0;
  AuthMsg->AmountApprovals = 0;
#endif
}
...
void TCheckAuthDlgClient::doAuthorization()
{
 ...
 // MAB072302        LastApproval,FirstApproval
 logg(__LINE__, "In chkauthdlg.cpp doAuthorization()", "",0,0);
 ...
 if ( declineRcode && ! approveRcode )
 {
   ...
   {
    // MAB072302        LastApproval,FirstApproval
    logg(__LINE__, "In chkauthdlg.cpp doAuthorization()", "Did isPositive() First approval",(long)AuthMsg->FirstApproval,0);
    logg(__LINE__, "In chkauthdlg.cpp doAuthorization()", "Did isPositive() Last Approval",(long)AuthMsg->LastApproval,0);
    logg(__LINE__, "In chkauthdlg.cpp doAuthorization()", "Did isPositive()Amt and sum",(long)AuthMsg->AmountApprovals, (int)AuthMsg->NumApprovals);
